# Constants
OPENAI_API_KEY = "REMOVED_OPENAI_KEY"  # Add your OpenAI API key here
...
def create_html_files(filename, overall_summary, chapters, content):
    ...
    for num, title in chapters:
        ...
        outline_html += f'<li><h2><a href="{chapter_filename}">Chapter {num}: {title}</a></h2><p>{chapter_summary}</p></li>'
    outline_html += f"</ul>\n{footer}"
    ...
    for num, title in chapters:
        chapter_html = f"{header}\n<h1>Chapter {num}: {title}</h1>\n<div class='chapter-content'></div>\n{footer}"
        with open(os.path.join(OUTPUT_FOLDER, f"{base_filename}_chapter_{num}.html"), 'w', encoding='utf-8') as f:
...
def process_file(file_path):
    ...
    create_html_files(filename, overall_summary, chapters, content)
    update_flask_app(filename, chapters)
    update_sitemap(filename, chapters)

    for num, title in chapters:
        logging.info(f"Processing Chapter {num}: {title}")
        chapter_content = extract_chapter_content(content, num, title)

        # Refine chapter content with AI
        refined_content = refine_chapter_content(chapter_content, num, title)

        # Remove unwanted HTML elements
        soup = BeautifulSoup(refined_content, 'html.parser')
        for unwanted in soup(['html', 'head', 'body']):
            unwanted.decompose()
        cleaned_content = str(soup)

        html_file = os.path.join(OUTPUT_FOLDER, f"{filename.lower().replace(' ', '_')}_chapter_{num}.html")
        with open(html_file, 'r', encoding='utf-8') as f:
            soup = BeautifulSoup(f, 'html.parser')
        content_div = soup.find('div', class_='chapter-content')
        content_div.clear()
        content_div.append(BeautifulSoup(cleaned_content, 'html.parser'))

        with open(html_file, 'w', encoding='utf-8') as f:
            f.write(str(soup))

        logging.info(f"Completed processing Chapter {num}")
