...
from flask import Flask, request, render_template, redirect, url_for, session, abort,Response,jsonify, request
import logging
import secrets
import math
import json
...
@app.route('/demo')
def demo():
    # Use already fetched slots or fetch new ones
    available_slots = session.get('available_slots')
    if not available_slots:
        fetch_available_slots()
        available_slots = session.get('available_slots')

    return render_template('demo.html', available_slots=available_slots)
